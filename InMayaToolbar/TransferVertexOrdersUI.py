...
class TransferVertexOrdersPluginUI( BasicUI ):
    def _showWindow( self, uiName ):
        ...
        def doIt():
            ...
            # 注冊事件，更新及完成事件
            eventId1 = om.MUserEventMessage.addUserEventCallback('onTransferVertexOrdersUpdate', onTransferVertexOrdersUpdateMethod)
            eventId2 = om.MUserEventMessage.addUserEventCallback('onTransferVertexOrdersDone', onTransferVertexOrdersDoneMethod)
            
            # 呼叫外挂      
            maya.cmds.TransferVertexOrders(passStr) 
               
            # 移除事件
            om.MUserEventMessage.removeCallback( eventId1 )
            om.MUserEventMessage.removeCallback( eventId2 )
